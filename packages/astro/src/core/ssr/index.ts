...
async function errorHandler(e: unknown, { viteServer, filePath, config }: ErrorHandlerOptions) {
  ...
  if (Array.isArray(anyError.errors)) {
    ...
    let src = anyError.pluginCode;
    if (!src && anyError.id && fs.existsSync(anyError.id)) src = await fs.promises.readFile(anyError.id, 'utf8');
...
`;
    ...
    throw err;
  } else if (anyError.stack) {
    const [message, maybeLoc] = eol.lf(anyError.stack).split('\n');
    ...
    if (fileMatch && fileMatch[1]) {
      ...
      if (!fs.existsSync(srcLoc)) srcLoc = fileURLToPath(new URL(srcLoc.replace(/^\//, ''), config.projectRoot));
...
export async function ssr(ssrOpts: SSROptions): Promise<string> {
  try {
    ...
    await errorHandler(e, { viteServer: ssrOpts.viteServer, filePath: ssrOpts.filePath, config: ssrOpts.astroConfig });
    throw e;
  }
}
